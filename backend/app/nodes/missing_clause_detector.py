...
def parse_json_safely(text: str, schema_key: str) -> Any:
    ...
    return [] if schema_key == "missing_clauses" else {}

def detect_missing_clauses(state: ContractState) -> Dict[str, Any]:
    # Load the missing clause detector prompt template
    prompt_template = (Path(__file__).parent.parent / "prompts" / "missing_clause.md").read_text()
    ...
    # Parse missing_clauses list safely using the robust JSON parser
    missing_clauses = parse_json_safely(response, "missing_clauses")
    
    # Ensure missing_clauses is a list
    if not isinstance(missing_clauses, list):
        missing_clauses = []
        
    return {"missing_clauses": missing_clauses}
